...
export function ArrowStats({ userScores }) {
    ...
    const getWeeksData = (bowType) => {
      ...
      for (let i = 1; i <= 8; i++) {
        ...
        const arrowsCount = filteredScores.reduce((count, score) => {
          const validArrows = score.arrowValues.filter(
            (arrow) =>
  ...
  const handleBowTypeSelection = (bowType) => {
    if (chartData[bowType].some((weekData) => weekData.arrows > 0)) {
      ...
      setAlertMessage(`No arrow data for selected bow type: ${bowType}`);
    }
  };

  const currentData = chartData[selectedBowType];
  const maxYValue = currentData
    ? Math.max(...currentData.map((item) => item.arrows))
    : 0;
    ...
    if (active && payload && payload.length) {
      return (
        ...
        >
          ...
          <p>
            <strong>Arrows:</strong> {payload[0].payload.arrows}
          </p>
        </div>
      );
    }
